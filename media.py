...
from config import PLEX_PRESORTED_FOLDER, PLEX_PRESORTED_IGNORE_FOLDERS, PLEX_LIBRARY_FOLDERS
...
class MediaListHandler(HandlerBaseClass):
    ...
    def handle(update, context):
        button_list = [
            get_inline_button(p, Emojis.FOLDER if p.is_dir()
                              else Emojis.FILM_CAMERA)
    ...
    def handle(update, context):
        ...
        for k, v in PLEX_LIBRARY_FOLDERS.items():
            button_list.append([telegram.InlineKeyboardButton(
                f'Move to "{v.name if type(v) == pathlib.PosixPath else v}"', callback_data=f"{k}_{file}")])
        reply_markup = telegram.InlineKeyboardMarkup(button_list)
        context.bot.send_message(
            chat_id=update.effective_chat.id,
...
class MoveFileToFolderCallbackCallbackHandler(HandlerBaseClass):
    ...
    pattern = reduce(
        lambda acc, k: f'{acc}{k}_.+|', PLEX_LIBRARY_FOLDERS.keys(), '')[:-1]
    ...
    def handle(update, context):
        ...
        for f in PLEX_PRESORTED_FOLDER.glob('*'):
            if f.name.startswith(file):
                f.replace(PLEX_PRESORTED_FOLDER /
                          folder / f.name)
                context.bot.send_message(
                    chat_id=update.effective_chat.id,
...
CALLBACKS = [MoveFileToFolderCallbackHandler,
             MoveFileToFolderCallbackCallbackHandler]
